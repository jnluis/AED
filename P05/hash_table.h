...
hash_table_node *find_hash_table_node(char key[64])
{
  ...
  // 1. usa  a key para calcular o idx
  // Se hash_table[idx] == NULL,
  // a chave não existe, retorna NULL
  unsigned int index  = hash_function(key);
  hash_table_node* node = hash_table[index];
  // else
  while( node!=NULL ){
  ...
  return NULL;
  // verifica se hash_table[idx]->key == key
  // se sim, retorna
  // se não, navega para a direita ate encontrar o nó com a chave ou NULL
}
...
{
  // place your code here
  // 1. int idx = hash_function(n->key)
  // 2. n->next = hash_table[idx];
  // 3. hash_table[idx] = n;
  unsigned int idx = hash_function( n->key );
